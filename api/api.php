...
return;
                case 'message': $this->handleMessage();
...
return;
    ...
    public function handleJoin()
    {
        ...
        $stmt = $this->pdo->prepare('DELETE FROM apns_users WHERE udid = ? AND app_id = ?');
        $stmt->execute([$udid, $appid]);

        $stmt = $this->pdo->prepare('INSERT INTO apns_users VALUES (?, ?, ?, ?)');
        $stmt->execute([$udid, $token, $appid, $_SERVER['REMOTE_ADDR']]);
    ...
    public function handleUpdate()
    {
        ...
        $appid = $this->getAppID();
        $stmt = $this->pdo->prepare('UPDATE apns_users SET device_token = ? WHERE udid = ? AND app_id = ?');
        $stmt->execute([$token, $udid, $appid]);
    }
    ...
    public function handleMessage()
    {
        ...
        $stmt = $this->pdo->prepare('SELECT * FROM apns_users WHERE udid = ? and app_id = ? and device_token <> ? LIMIT 1');
        $stmt->execute([$udid, $appid, $token]);
        ...
        if ($user !== false) {
            ...
            $stmt = $this->pdo->prepare('SELECT device_token FROM apns_users WHERE app_id = ?');
            $stmt->execute([$appid]);
    ...
    public function getAppID()
    {
        if (!isset($_POST['appid'])) {
            exitWithHttpError(400, 'Missing appid');
        }

        $appid = trim(urldecode($_POST['appid']));
    ...
    public function isValidUDID($udid)
    {
    ...
    // appear to be valid, the script exits with an error message.
    public function getDeviceToken($mayBeEmpty = false)
    {
    ...
    public function isValidDeviceToken($deviceToken)
    {
    ...
    // an error message.
    public function getString($name, $maxLength, $allowNewlines = false)
    {
        if (!isset($_POST[$name])) {
    ...
    public function makePayload($senderName, $text)
    {
        ...
        $payload = '{"aps":{"alert":"'.$textJson.'","sound":"default"}}';
    ...
    public function addPushNotification($deviceToken, $payload)
    {
        ...
        if (strlen($payload) <= 256) {
            $stmt = $this->pdo->prepare('INSERT INTO apns_queue (device_token, payload, time_queued) VALUES (?, ?, NOW())');
            $stmt->execute([$deviceToken, $payload]);
        }
    }
}
