...
class API
{
	...
	function handleJoin()
	{
		...
		$stmt->execute(array($udid, $appid));

		$stmt = $this->pdo->prepare('INSERT INTO apns_users VALUES (?, ?, ?, ?)');
		$stmt->execute(array($udid, $token, $appid, $_SERVER['REMOTE_ADDR']));
	...
	function handleUpdate()
	{
		...
		$stmt->execute(array($token, $udid, $appid));
	}
	...
	function handleMessage()
	{
		...
		$stmt->execute(array($udid, $appid, $token));
