...
class APNS_Push
{
    ...
    public function start()
    {
        ...
        while (true) {
            ...
            $stmt = $this->pdo->prepare('SELECT * FROM apns_queue WHERE time_sent IS NULL LIMIT 20');
            $stmt->execute();
            $messages = $stmt->fetchAll(PDO::FETCH_OBJ);

            foreach ($messages as $message) {
                if ($this->sendNotification($message->message_id, $message->device_token, $message->payload)) {
                    $stmt = $this->pdo->prepare('UPDATE apns_queue SET time_sent = NOW() WHERE message_id = ?');
    ...
    public function sendNotification($messageId, $deviceToken, $payload)
    {
